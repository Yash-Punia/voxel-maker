...
- **Every clickable trigger (`<button>`, or `<div>`/`<span>` with `role="button"`) must give visual press feedback.** Use `active:scale-[0.98]` + `transition-all` so the element briefly shrinks while the mouse is pressed. Shared classes already include it (`.btn`, `.tool-btn`, `.swatch`) — if you write a one-off button, append these utilities inline. Never ship a click target with zero press feedback; it feels broken.
- **Native form controls and their wrapping `<label>`s are exempt** from the scale rule — `<input type="checkbox|radio|color|range">` and `<select>` have built-in press states. Scaling their labels feels noisy.
